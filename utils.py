...
def get_clean_factor_and_forward_returns(
    ...
    if not isinstance(factor.index, pd.MultiIndex):
        factor = factor.stack()
    new_levels = pd.to_datetime(factor.index.levels[0]).tz_localize('UTC')
    factor.index = factor.index.set_levels(new_levels, level=0)
...
def get_long_short_info(ic_data:pd.DataFrame, quantile_max:int):
    """
    Get long-short grouping information, which is determined by the positive and negative IC values
    Parameters
    ----------
    ...
    pd.DataFrame
        _description_
    """
    long_short_info = pd.DataFrame(ic_data)
    long_short_info["long"] = ic_data.apply(lambda x: quantile_max if x else 1)
    long_short_info["short"] = ic_data.apply(lambda x:  1 if x else quantile_max)
    long_short_info.drop(labels=0, axis=1, inplace=True)
    return long_short_info

def get_long_portfolio_annual_transaction_cost(turnover_table:pd.DataFrame, long_short_info:pd.DataFrame):
    """
    Get long portfolio transaction costs
    Parameters
    ----------
    turnover_table : pd.DataFrame
        _description_
    long_short_info : pd.DataFrame
        _description_
    ...
    pd.DataFrame
        _description_
    """
    long_portfolio_turnover =  turnover_table.apply(lambda x: x.iloc[long_short_info.loc[x.name]["long"]-1])
    transfer_data = list(map(lambda x: pd.Timedelta("252D") / pd.Timedelta(x), long_portfolio_turnover.index))
    long_portfolio_annual_transaction_cost = long_portfolio_turnover.multiply(transfer_data) *2*0.002
    return long_portfolio_annual_transaction_cost