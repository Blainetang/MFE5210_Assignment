...
def factor_weights(factor_data, demeaned=True, group_adjust=False, equal_weight=True):
    """
...
def cumulative_returns(returns, period):
    ...
    if not isinstance(period, int):
        raise TypeError(f"Expected a int, but got a {type(period).__name__}")
    ...
    sub_portfolios = sub_portfolios.rolling(period, min_periods=1, center=False)\
...
def mean_return_by_quantile(
    ...
    mean_ret : pd.DataFrame
        Mean period wise annual returns by specified factor quantile.
    std_error_ret : pd.DataFrame
        Standard error of returns by specified quantile.
    """
    if by_year and not demeaned:
        raise ValueError("When by_year is True, demeaned must also be True.")
    ...
    if not by_date:
        ...
        if by_year:
            grouper.append(list(map(lambda x: x.year, mean_ret.index.get_level_values(level="date"))))
    ...
    if by_year:
        # 年化
        return mean_ret.apply(utils.rate_of_return, axis=0, base_period="252D")
    else:
...
def get_returns_info(mean_quant_ret_bydate, long_short_info):
    cols = utils.get_forward_returns_columns(mean_quant_ret_bydate.columns)
    grouper = [mean_quant_ret_bydate.index.get_level_values(level="factor_quantile")]
    quantile_return_dict = {}
    factor_return_dict = {}
    for col in cols:
        quantile_return_dict[col] = mean_quant_ret_bydate.groupby(grouper)[col]\
            .apply(lambda x: cumulative_returns(x, int(col[:-1]))).droplevel(level=0).unstack().T
        factor_return_dict[col] = quantile_return_dict[col][long_short_info.loc[col]["long"]] /\
            quantile_return_dict[col][long_short_info.loc[col]["short"]]
    return quantile_return_dict, pd.concat(factor_return_dict, axis=1)