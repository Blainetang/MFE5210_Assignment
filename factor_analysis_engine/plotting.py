...
def plot_returns_table(alpha_beta, mean_ret_quantile, mean_ret_spread_quantile):
    ...
    returns_table.loc["Mean Period Wise Return Top Quantile (%)"] = (
        mean_ret_quantile.iloc[-1] * DECIMAL_TO_PERCENT
    )
    returns_table.loc["Mean Period Wise Return Bottom Quantile (%)"] = (
        mean_ret_quantile.iloc[0] * DECIMAL_TO_PERCENT
    )
    returns_table.loc["Mean Period Wise Spread (%)"] = (
        mean_ret_spread_quantile.mean() * DECIMAL_TO_PERCENT
    )
...
def plot_information_table(ic_data):
    ...
    ic_summary_table["IC_IR"] = ic_data.mean() / ic_data.std()
...
def plot_ic_ts(ic, ax=None):
    ...
    for a, (period_num, ic) in zip(ax, ic.items()):
        ...
        ic.rolling(window=22).mean().plot(ax=a, linestyle="--", color="forestgreen", lw=2, alpha=0.5)
        ax1 = a.twinx()
        ic.cumsum().plot(ax=ax1, linestyle="-.", color="red", lw=2, alpha=0.8)
        ...
        ax1.legend(lines + lines2, ["IC", "MA20", "CUMSUM"], loc="upper right", fontsize=10)
        a.text(
            0.75,
            0.96,
            "Mean %.3f \n Std. %.3f \n IC_IR %.3f" % (ic.mean(), ic.std(), ic.mean()/ic.std()),
            fontsize=11,
...
def plot_quantile_returns_bar(
    ...
    if ylim_percentiles is not None:
        ymin = (
            np.nanpercentile(mean_ret_by_q.values, ylim_percentiles[0]) * DECIMAL_TO_PERCENT
        )
        ymax = (
            np.nanpercentile(mean_ret_by_q.values, ylim_percentiles[1]) * DECIMAL_TO_PERCENT
        )
    else:
        ymin = None
        ymax = None
    ...
    if by_group:
        ...
        for a, (sc, cor) in zip(ax, mean_ret_by_q.groupby(level="group")):
            (
                ...
                .multiply(DECIMAL_TO_PERCENT)
                .plot(kind="bar", title=sc, ax=a)
            )

            a.set(xlabel="", ylabel="Mean Return (%)", ylim=(ymin, ymax))
        ...
        (
            mean_ret_by_q.multiply(DECIMAL_TO_PERCENT).plot(
                kind="bar",
                title="Mean Period Wise Return By Factor Quantile",
                ax=ax,
            )
        )
        ax.set(xlabel="", ylabel="Mean Return (%)", ylim=(ymin, ymax))
...
def plot_quantile_returns_violin(return_by_q, ylim_percentiles=None, ax=None):
    ...
    if ylim_percentiles is not None:
        ymin = (
            np.nanpercentile(return_by_q.values, ylim_percentiles[0]) * DECIMAL_TO_PERCENT
        )
        ymax = (
            np.nanpercentile(return_by_q.values, ylim_percentiles[1]) * DECIMAL_TO_PERCENT
        )
    else:
        ymin = None
        ymax = None
    ...
    unstacked_dr = return_by_q.multiply(DECIMAL_TO_PERCENT)
...
def plot_quantile_returns_bar_by_year(mean_return_by_year, ax=None):
    mean_return_by_year = mean_return_by_year.copy()
    mean_return_by_year = mean_return_by_year.multiply(DECIMAL_TO_PERCENT)
    if ax is None:
        f, ax = plt.subplots(1, len(mean_return_by_year.columns), figsize=(18, 6))
    for i, col in enumerate(mean_return_by_year.columns):
        temp_df = mean_return_by_year[col].unstack()
        ax[i].bar(mean_return_by_year.index, mean_return_by_year[col])
        ax.set(
        xlabel="",
        ylabel="Return (%)",
        title="Mean Return by Year"
        )
    return ax
...
def plot_mean_quantile_returns_spread_time_series(
    ...
    mean_returns_spread_percent = mean_returns_spread * DECIMAL_TO_PERCENT

    mean_returns_spread_percent.plot(alpha=0.4, ax=ax, lw=0.7, color="forestgreen")
    mean_returns_spread_percent.rolling(window=22).mean().plot(
        color="orangered", alpha=0.7, ax=ax
    )
    ax.legend(["mean returns spread", "1 month moving avg"], loc="upper right")

    if std_err is not None:
        std_err_percent = std_err * DECIMAL_TO_PERCENT
        upper = mean_returns_spread_percent.values + (std_err_percent * bandwidth)
        lower = mean_returns_spread_percent.values - (std_err_percent * bandwidth)
        ax.fill_between(
    ...
    ylim = np.nanpercentile(abs(mean_returns_spread_percent.values), 95)
    ax.set(
        ylabel="Difference In Quantile Mean Return (%)",
        xlabel="",
        title=title,
        ylim=(-ylim, ylim),
    )
...
def plot_cumulative_returns(factor_returns_df, freq=None, title=None, ax=None):
    """
    ...
    factor_returns_df.plot(ax=ax, lw=3, alpha=0.6)
    ax.set(
        ylabel="Cumulative Returns",
        title=(
            "Portfolio Cumulative Return in Different Periods"
            if title is None
            else title
        ),
        xlabel="",
...
def plot_quantile_returns_bar_by_year(mean_return_by_year, ax=None):
    ...
    if ax is None:
        f, ax = plt.subplots(1, len(mean_return_by_year.columns), figsize=(18, 6))
    for i, col in enumerate(mean_return_by_year.columns):
        mean_return_by_year[col].unstack().plot(kind="bar", ax=ax[i])
        ax[i].set(
        xlabel="",
        ylabel="Return (%)",
        title="{} Period Mean Return by Year ".format(col)
        )
    return ax

def plot_quantile_cumulative_returns(quantile_excess_return_dict, ax=None, type_="normal"):
    if ax is None:
        f, ax = plt.subplots(1, len(quantile_excess_return_dict.keys()), figsize=(18, 6))
    for i, key in enumerate(quantile_excess_return_dict.keys()):
        quantile_excess_return_dict[key].plot(ax=ax[i])
        if type_ == "normal":
            ax[i].set(
            xlabel="",
            ylabel="Return",
            title="{} Period Quantile Return".format(key)
            )
        else:
            ax[i].set(
            xlabel="",
            ylabel="Excess Return",
            title="{} Period Quantile Excess Return".format(key)
            )
        ax[i].legend(loc='upper left', framealpha=0.5)
    return ax
...
def plot_quantile_average_cumulative_return(
    ...
    avg_cumulative_returns = avg_cumulative_returns.multiply(DECIMAL_TO_PERCENT)
